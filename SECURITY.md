...
Expojet classifies `EXPO_PUBLIC_*` values as public bundle configuration. Clerk secret keys, database URLs, authentication secrets, and migration credentials are server-only and must never be written under a generated mobile workspace. Rotate any credential pasted into chat, logs, commits, or issue trackers.
